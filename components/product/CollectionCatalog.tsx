...
export default function CollectionCatalog({ collection }: { collection: CollectionKey }) {
  ...
  // Generate dynamic filters from products in this collection
  const dynamicFilters = useMemo(() => {
    if (!collectionProducts.length) return { colors: ["All"], sizes: ["All"], priceRanges: ["All"] }
    ...
    const priceStep = Math.ceil((maxPrice - minPrice) / 3) || 1
    const priceRanges = [
      "All",
  ...
  const filteredProducts = useMemo(() => {
    return collectionProducts.filter((product: Product) => {
      // Search filter
  ...
  }, [collectionProducts, searchTerm, selectedColor, selectedSize, selectedPrice, dynamicFilters])
  ...
  return (
    <div ref={containerRef} className="bg-white text-neutral-900 overflow-hidden">
      ...
      <section className="relative h-96 flex items-center justify-center overflow-hidden pt-20 bg-neutral-50">
        ...
        >
          <motion.h1
            className="font-nike-display text-5xl uppercase tracking-[0.04em] text-neutral-900 md:text-7xl mb-4"
